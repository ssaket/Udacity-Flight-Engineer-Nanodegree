...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        ...
        if self.flight_state == States.WAYPOINT:
            if np.linalg.norm(self.target_position[0:2] - self.local_position[0:2]) < 1.0:
                altitude = -1 * self.local_position[2]
                if (altitude > 0.95 * self.target_position[2]) and len(self.all_waypoints) > 0:
                    self.waypoint_transition()
    ...
    def velocity_callback(self):
        ...
        if self.flight_state == States.LANDING:
            if(abs(self.global_position[2] - self.global_home[2]) < 0.2 and abs(self.local_position[2] < 0.02)):
    ...
    def calculate_box(self):
        ...
        """
        local_waypoints = [[10, 0, 3], [10, -10, 3], [0, -10, 3], [0, 0, 3]]
